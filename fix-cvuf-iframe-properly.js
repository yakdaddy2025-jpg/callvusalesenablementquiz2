/**
 * Replace ALL inline voice recorder HTML/JS with iframes pointing to Vercel embed
        ...
        block.rows.forEach((row) => {
          if (row.fields) {
            row.fields.forEach((field) => {
              ...
              // Look for fields that contain script tags or voice recorder content
              if (field.type === 'paragraph' && field.editedParagraph) {
                const content = field.editedParagraph;
                
                // Check if it contains voice recorder indicators
                if (content.includes('<script>') || 
                    content.includes('Voice Recorder') || 
                    content.includes('voice-recorder') ||
                    content.includes('toggleRecording') ||
